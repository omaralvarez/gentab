...
class DataFrameParser(object):
    ...
    def fit(self, dataframe, threshold):
        ...
        self._original_column_to_dtype = (
            column_to_dtype
        ) = self.new_dataframe.dtypes.to_dict()
        ...
        for column, datatype in column_to_dtype.items():
            if datatype in ["O", "<U32"]:
                ...
                np.issubdtype(datatype, np.integer)
                and self.new_dataframe[column].nunique() == 2
            ):
                self.binary_columns.append(column)
                ...
                np.issubdtype(datatype, np.integer)
                and self.new_dataframe[column].nunique() <= 25
                ...
                repeated_entries = counts[
                    counts > threshold * len(self.new_dataframe[column])
                ].index.tolist()
        ...
        self._column_order = (
            self.binary_columns + self.categorical_columns + self.numerical_columns
        )
        ...
        for column in self.binary_columns:
            if self.new_dataframe[column].dtype == int:
                encoders[column] = LabelEncoder().fit_bin_int(
                    self.new_dataframe[column].astype(int)
                )
            else:
                encoders[column] = LabelEncoder().fit(
                    self.new_dataframe[column].astype(str)
                )

        for column in self.categorical_columns:
            if column in self.need_freq_encoding:
                encoders[column] = FreqLabelEncoder().fit(
                    self.new_dataframe[column].astype(str)
                )
    ...
    def invert_fit(self, encoded_table):
        ...
        for column, encoder in self.encoders.items():
            if column in self.numerical_columns:
                decoded_table[column] = StandardScaler().fit_invert(
                    self.new_dataframe[column], encoded_table[column]
                )

            elif (
                column in self.binary_columns
                and np.issubdtype(self.new_dataframe[column].dtype, np.integer) == True
            ):
                decoded_table[column] = encoded_table[column].astype(int)

            elif column in self.need_int_encoding:
                decoded_table[column] = LabelEncoder().fit_invert(
                    self.new_dataframe[column].astype(int), encoded_table[column]
                )

            else:
                decoded_table[column] = LabelEncoder().fit_invert(
                    self.new_dataframe[column].astype(str), encoded_table[column]
                )
...
def convert_to_table(org_df, gen_output, threshold):
    ...
    def sigmoid_threshold(logits):
        ...
        threshold_output = torch.where(
            sigmoid_output > 0.5, torch.tensor(1), torch.tensor(0)
        )
    ...
    if len(cards) != 0:
        for idx in range(len(cards)):
            cat_tensor[:, idx] = (
                softmax_with_max(gen_output["cats"][idx]).detach().to(torch.int64)
            )

    if n_bins != 0:
        for idx in range(n_bins):
            bin_tensor[:, idx] = (
                sigmoid_threshold(gen_output["bins"][:, idx]).detach().to(torch.int64)
            )
