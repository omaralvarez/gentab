...
def compute_v(ll, alpha, beta):
    ...
    v[torch.gt(ll, beta)] = torch.tensor(0.0, device=v.device)
    v[torch.le(ll, alpha)] = torch.tensor(1.0, device=v.device)

    if ll[torch.eq(v, -1)].shape[0] != 0 and ll[torch.eq(v, -1)].shape[0] != 1:
        v[torch.eq(v, -1)] = min_max_scaling(ll[torch.eq(v, -1)], scale=(1, 0)).to(
            v.device
        )
    else:
...
def loss_fn(model, Input_Data, T, eps=1e-5):
    ...
    for row in range(N):
        random_t = torch.rand(T) * (1.0 - eps) + eps
...
def Euler_Maruyama_sampling(model, T, N, P, device):
    time_steps = torch.linspace(1.0, 1e-5, T)
